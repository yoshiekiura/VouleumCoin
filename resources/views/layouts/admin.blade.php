...
<head>
    ...
    {{-- Site Title  --}}
    <title> @yield('title') | {{ config('settings.site_name', config('app.name', 'TokenLite')) }}</title>
    {{-- Vendor Bundle CSS --}}
...
<body class="admin-dashboard page-user">
                        ...
                        <li class="topbar-nav-item relative">
                            ...
                            <div class="toggle-class dropdown-content dropdown-content-right dropdown-arrow-right user-dropdown">
                                <div class="user-status">
                                    <h6 class="user-status-title">{{ Auth::user()->name }}<span>({{ 'UD'.sprintf('%04s', Auth::id()) }})</span></h6>
            ...
            <div class="container">
                <div class="navbar-innr">
                    <ul class="navbar-menu" id="main-nav">
                        ...
                        <li><a href="{{ route('admin.transactions.withState', 'pending') }}"><em class="ikon ikon-transactions"></em> Transactions</a></li>
                        <li><a href="{{ route('admin.kycs') }}"><em class="ikon ikon-docs"></em> KYC List</a></li>
                        <li><a href="{{ route('admin.users') }}"><em class="ikon ikon-user-list"></em> Users List</a></li>
                        <li><a href="{{ route('admin.stages') }}"><em class="ikon ikon-coins"></em> ICO Stage</a></li>

                        <li class="has-dropdown page-links-all"><a class="drop-toggle" href="javascript:void(0)"><em class="ikon ikon-settings"></em> Management</a>
                            <ul class="navbar-dropdown">
                                <li><a href="{{ route('admin.stages.settings') }}"> ICO Settings</a></li>

                                @if(is_super_admin())
                                <li><a href="{{ route('admin.settings') }}">Website Settings</a></li>
                                <li><a href="{{ route('admin.settings.email') }}">Mailing Settings</a></li>
                                <li><a href="{{ route('admin.payments.setup') }}">Payment Setup</a></li>
                                @endif

                                <li><a href="{{ route('admin.pages') }}"> Page Setting</a></li>
    ...
    <div class="footer-bar">
        <div class="container">
            <div class="row align-items-center justify-content-center">
                <div class="col-md-12">
                    <div class="copyright-text text-center pb-3">&copy; {{ date('Y ').'TokenLite. All Rights Reserved. Application Developed by ' }}<a href="//softnio.com" target="_blank">Softnio</a>.</div>
                </div>
            </div>
        </div>{{-- .container --}}
    </div>{{-- .footer-bar --}}
    ...
    <script type="text/javascript">
        ...
        get_trnx_url = "{{ route('admin.ajax.transactions.view') }}",
        view_user_url = "{{ route('admin.ajax.users.view') }}",
        pm_manage_url = "{{ route('admin.ajax.payments.view') }}",
        get_kyc_url = "{{ route('admin.ajax.kyc.ajax_show') }}",
        update_kyc_url = "{{route('admin.ajax.kyc.update')}}",
        trnx_action_url = "{{ route('admin.ajax.transactions.update') }}",
        trnx_adjust_url = "{{ route('admin.ajax.transactions.adjustement') }}",
        get_et_url = "{{ route('admin.ajax.settings.email.template.view') }}",
        clear_cache_url = "{{ route('admin.clear.cache') }}",
        whitepaper_uploads = "{{ route('admin.ajax.pages.upload') }}",
        view_page_url = "{{ route('admin.ajax.pages.view') }}";
        unverified_delete_url = "{{ route('admin.ajax.users.delete') }}";
        active_stage_url = "{{ route('admin.ajax.active.stage') }}" ;
        pause_stage_url = "{{ route('admin.ajax.pause.stage') }}";
    </script>
