...
class EmailSettingController extends Controller
{
    ...
    public function show_template(Request $request)
    {
        ...
        if ($request->input('get_template') == null) {
            ...
            if ($template) {
                ...
                return response()->json(['msg'=>'warning', 'message'=>__('messages.something_wrong')]);
            }
        }
    }
    ...
    public function update_template(Request $request)
    {
        ...
        if ($validator->fails()) {
            $msg = '';
            if ($validator->errors()->has('slug') || $validator->errors()->has('name') || $validator->errors()->has('subject')) {
                $msg = $validator->errors()->first();
            } else {
                $msg = __('messages.something_wrong');
            }
